...
namespace RHSMCLO001
{
    public partial class frmClasificacionOcupacional : Form
    {
        ...
        private void On_Save(object sender, EventArgs e)
        {
            try
            {
                if (txt.Text != "")
                {
                    ...
                    ControllerRHSMCLO001 controler = new ControllerRHSMCLO001();
                    controler.AddClasificacionOcupacional(objData, conection);
                    UpdateLookup();
                }
        ...
        private bool On_Delete(object sender, EventArgs e)
        {
            try
            {
                if (txt.Text != "")
                {
                    bool result;
                    ControllerRHSMCLO001 controler = new ControllerRHSMCLO001();
                    result = controler.DeleteClasificacionOcupacional(txt.Text, conection);
                    if (result)
                    {
                        UpdateLookup();
                        On_Cancel(null, null);
        ...
        private void On_IDChange(object sender, EventArgs e)
        {
            ...
            {
                ControllerRHSMCLO001 controler = new ControllerRHSMCLO001();
                ThrOcupationClasification data = controler.GetClasificacionOcupacion(txt.Text, conection);
                if (data != null)
                {
                    MainBS.Add(data);
        ...
        private void LookupButton1_OnLookupReturn(object sender, Net4Sage.Controls.Lookup.LookupReturnEventArgs eventArgs)
        {
            ...
            try
            {
                ...
                {
                    ControllerRHSMCLO001 controler = new ControllerRHSMCLO001();
                    var clasf = controler.GetClasificacionOcupacion(txt.Text, conection);
